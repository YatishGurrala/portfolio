...
export function SiteHeader() {
  return (
    <header className="sticky top-0 z-40 border-b border-white/60 bg-[color:var(--background)]/90 backdrop-blur dark:border-white/10">
          ...
          <nav aria-label="Primary" className="overflow-x-auto">
            <ul className="flex min-w-max items-center gap-2 text-sm font-medium text-slate-700 dark:text-slate-200">
              {siteConfig.navigation.map((item) => (
                <li key={item.href}>
                  <Link className="inline-flex rounded-full px-3 py-2 transition-colors hover:bg-white/80 dark:hover:bg-slate-900/80" href={item.href}>
                    {item.label}
                  </Link>
                </li>
              ))}
